...
def test_weights_w(IA_mock_TNG300_n1):
	'''
	Compare limited TNG300 catalogue outputs with saved version.
	:param IA_mock_TNG300:
	:return:
	'''
	IA_mock_TNG300_n1.measure_xi_w("All_both", 'both', 8, temp_file_path='./data/processed/')
	IA_mock_TNG300_n1.data["weight"] = np.array([0.5] * len(IA_mock_TNG300_n1.data["Position"][:, 0]))
	IA_mock_TNG300_n1.data["weight_shape_sample"] = np.array(
		[0.5] * len(IA_mock_TNG300_n1.data["Position_shape_sample"][:, 0]))
	IA_mock_TNG300_n1.measure_xi_w("All_both_weight", 'both', 8, temp_file_path='./data/processed/')
	...
	np.testing.assert_allclose(cov_wgp, 16 * a_cov_wgp)
	np.testing.assert_allclose(cov_wgg, 16 * a_cov_wgg)

	return


def test_weights_m(IA_mock_TNG300_n1):
	'''
	Compare limited TNG300 catalogue outputs with saved version.
	:param IA_mock_TNG300:
	:return:
	'''
	IA_mock_TNG300_n1.measure_xi_multipoles("All_both", 'both', 8, temp_file_path='./data/processed/')
	IA_mock_TNG300_n1.data["weight"] = np.array([0.5] * len(IA_mock_TNG300_n1.data["Position"][:, 0]))
	IA_mock_TNG300_n1.data["weight_shape_sample"] = np.array(
		[0.5] * len(IA_mock_TNG300_n1.data["Position_shape_sample"][:, 0]))
	IA_mock_TNG300_n1.measure_xi_multipoles("All_both_weight", 'both', 8, temp_file_path='./data/processed/')
