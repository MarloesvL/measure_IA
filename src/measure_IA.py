...
class MeasureVariablesSnapshot(SimInfo):
    ...
    def __init__(self, PT=4, project="TNG100", snapshot=None, output_file_name=None, data_path="./data/raw/"):
    ...
    def measure_spin(self):
        ...
        for n in np.arange(0, self.Num_halos):
            ...
            if self.PT == 1:
                ...
                rel_position = TNG100_snapshot.read_cat(self.coordinates_name, [off_n, off_n + len_n]) - COM[n]
                rel_velocity = (
                    TNG100_snapshot.read_cat(self.velocities_name, [off_n, off_n + len_n]) * np.sqrt(self.scalefactor)
                    - velocity[n]
                )
            elif self.PT == 4:
                wind_or_star = TNG100_snapshot.read_cat(self.wind_name, cut=[off_n, off_n + len_n])
        ...
        if corrtype == "auto":
            RR = (
                ...
                * (
                    ...
                    - (self.get_volume_spherical_cap(mur_min, r_min) - self.get_volume_spherical_cap(mur_max, r_min))
                )
                / L3
            )
