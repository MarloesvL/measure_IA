		...
		if (self.output_file_name != None) and (return_output == False):
			...
			if grids.Splus_Splus is not None:
				self.write_shape_shape_grids(output_file, "multipoles", ("r", "mu_r"), grids,
											 ...
											 mu_r_bins, dataset_name)
				self.write_shape_shape_jk_realisations(
					output_file, "multipoles", ("r", "mu_r"), grids, RR_jk,
					(2 * R) * (2 * R_pos), R_jk, R_pos_jk, separation_bins, mu_r_bins,
		...
		if (self.output_file_name != None) and (return_output == False):
			...
			if grids.Splus_Splus is not None:
				...
				self.write_shape_shape_jk_realisations(
					...
					mu_r_bins, dataset_name, jk_group_name, self.num_box)
			output_file.close()
			return
		else:
			return correlation, xi_gg, separation_bins, mu_r_bins, Splus_D, DD, RR_g_plus
