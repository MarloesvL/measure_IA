...
class MeasureMBoxJackknife(MeasureIABase, ReadData):
	...
	def _measure_xi_r_mur_box_jk_batch(self, i):
		r"""(r, mu_r) jackknife shape-sample batch worker. Reads shared memory (incl. jk region
		indices) and delegates the counting + union-deletion jk accumulation to pair_kernel.accumulate
		(BoxRMuR, jk=True, reusing the parent's shared self.pos_tree). Support function for
		_measure_xi_r_mur_box_jk_multiprocessing().
		"""
	...
	def _count_pairs_xi_r_mur_box_jk_brute(self, dataset_name, L_subboxes, masks=None, rp_cut=None,
										   return_output=False, jk_group_name=""):
		r"""Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_brute for corr_type='gg':
	...
	def _count_pairs_xi_r_mur_box_jk_tree(self, dataset_name, L_subboxes, masks=None, rp_cut=None,
										  return_output=False, jk_group_name=""):
		r"""Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_tree for corr_type='gg':
	...
	def _count_pairs_xi_r_mur_box_jk_batch(self, i):
		r"""(r, mu_r) DD-only jackknife shape-sample batch worker. Reads shared memory (incl. jk region
		indices) and delegates to pair_kernel.accumulate (BoxRMuR, shapes=False, jk=True). Support function
		for _count_pairs_xi_r_mur_box_jk_multiprocessing().
		"""
		if i + self.chunk_size > self.Num_shape_masked:
			i2 = self.Num_shape_masked
		else:
	...
	def _count_pairs_xi_r_mur_box_jk_multiprocessing(self, dataset_name, L_subboxes, temp_file_path, masks=None,
													 rp_cut=None, return_output=False, jk_group_name="",
													 chunk_size=1000, num_nodes=1):
		r"""Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_multiprocessing for corr_type='gg':
		skips all shape/ellipticity computation. Uses >1 CPU. Uses KDTree for speedup.
		...
		if "." in dataset_name:
			figname_dataset_name = figname_dataset_name.replace(".", "p")
		file_temp = h5py.File(f"{temp_file_path}/multipoles_gg_{self.simname}_temp_data_{figname_dataset_name}.hdf5", "w")
		keys = []
		for k in self.data.keys():
			if k != "LOS":
				write_dataset_hdf5(file_temp, k, self.data[k])
		...
		try:
			...
			if os.path.exists(f"{temp_file_path}/multipoles_gg_{self.simname}_temp_data_{figname_dataset_name}.hdf5"):
				temp_data_obj_m = ReadData(self.simname, f"multipoles_gg_{self.simname}_temp_data_{figname_dataset_name}", None,
										   data_path=temp_file_path)
				for k in keys:
					self.data[k] = temp_data_obj_m.read_cat(k)
					if masks is not None:
	...
	def _legacy_measure_xi_r_mur_box_jk_brute(self, dataset_name, L_subboxes, masks=None, rp_cut=None, return_output=False,
									   jk_group_name="", ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions including jackknife realisations, $\xi_{gg}$ and $\xi_{g+}$,
		in (r, mu_r) bins for an object created with MeasureIABox. Uses 1 CPU.
	...
	def _legacy_measure_xi_r_mur_box_jk_tree(self, dataset_name, L_subboxes, masks=None, rp_cut=None, return_output=False,
									  jk_group_name="", ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions including jackknife realisations, $\xi_{gg}$ and $\xi_{g+}$,
		in (r, mu_r) bins for an object created with MeasureIABox. Uses 1 CPU. Uses KDTree for speedup.
	...
	def _legacy_measure_xi_r_mur_box_jk_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures components of $\xi_{gg}$ and $\xi_{g+}$ in (r, mu_r) bins including jackknife realisations for a
	...
	def _legacy_measure_xi_r_mur_box_jk_multiprocessing(self, dataset_name, L_subboxes, temp_file_path, masks=None,
												 rp_cut=None, return_output=False, jk_group_name="",
												 chunk_size=100, num_nodes=1, ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions including jackknife realisations, $\xi_{gg}$ and $\xi_{g+}$,
		in (r, mu_r) bins for an object created with MeasureIABox. Uses >1 CPU. Uses KDTree for speedup.
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_measure_xi_r_mur_box_jk_batch, indices)
	...
	def _legacy_count_pairs_xi_r_mur_box_jk_brute(self, dataset_name, L_subboxes, masks=None, rp_cut=None,
										   return_output=False, jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_brute for corr_type='gg':
		skips all shape/ellipticity computation. Uses 1 CPU.
	...
	def _legacy_count_pairs_xi_r_mur_box_jk_tree(self, dataset_name, L_subboxes, masks=None, rp_cut=None,
										  return_output=False, jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_tree for corr_type='gg':
		skips all shape/ellipticity computation. Uses 1 CPU. Uses KDTree for speedup.
		...
		for i in np.arange(0, len(positions_shape_sample), 100):
			...
			ind_max_i = shape_tree.query_ball_tree(pos_tree, self.r_max)
			ind_rbin_i = self.setdiff2D(ind_max_i, ind_min_i)
			for n in np.arange(0, len(positions_shape_sample_i)):  # loop over shapes, not positions
				if len(ind_rbin_i[n]) > 0:
					separation = positions_shape_sample_i[n] - positions[ind_rbin_i[n]]
					if self.periodicity:
						separation[separation > self.L_0p5] -= self.boxsize  # account for periodicity of box
					...
					pos_mask = \
						np.where(
							...
							0]
					np.add.at(DD_jk, (jackknife_region_indices_shape_i[n], ind_r, ind_mu_r),
							  (weight[ind_rbin_i[n]][mask] * weight_shape_i[n]))
					np.add.at(DD_jk,
							  (jackknife_region_indices_pos[ind_rbin_i[n]][mask][pos_mask], ind_r[pos_mask],
							   ind_mu_r[pos_mask]),
							  (weight[ind_rbin_i[n]][mask][pos_mask] * weight_shape_i[n]))
	...
	def _legacy_count_pairs_xi_r_mur_box_jk_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures the weighted pair counts DD in (r, mu_r) bins including jackknife realisations for a batch of
		indices from i to i+chunk_size. Support function for _count_pairs_xi_r_mur_box_jk_multiprocessing().
		...
		-------
		ndarrays
			DD, DD_jk where DD_jk stores the necessary information of DD for each jackknife realisation.
		"""
		if i + self.chunk_size > self.Num_shape_masked:
			i2 = self.Num_shape_masked
		else:
				...
				if len(ind_rbin_i[n]) > 0:
					...
					pos_mask = \
						np.where(
							...
							0]
					np.add.at(DD_jk, (jackknife_region_indices_shape_i[n], ind_r, ind_mu_r),
							  (weight_i_n[mask] * weight_shape_i[n]))
					np.add.at(DD_jk,
							  (jackknife_region_indices_pos[ind_rbin_i[n]][mask][pos_mask], ind_r[pos_mask],
							   ind_mu_r[pos_mask]),
							  (weight_i_n[mask][pos_mask] * weight_shape_i[n]))
					del mask, separation_len, mu_r
		for shm in shms:
			shm.close()
		return DD, DD_jk

	def _legacy_count_pairs_xi_r_mur_box_jk_multiprocessing(self, dataset_name, L_subboxes, temp_file_path, masks=None,
													 rp_cut=None, return_output=False, jk_group_name="",
													 chunk_size=1000, num_nodes=1):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, including jackknife realisations in (r, mu_r) bins for an object
		created with MeasureIABox. DD-only twin of _measure_xi_r_mur_box_jk_multiprocessing for corr_type='gg':
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_count_pairs_xi_r_mur_box_jk_batch, indices)
