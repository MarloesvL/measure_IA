...
class MeasureWLightconeJackknife(MeasureIABase):
	...
	def _measure_xi_rp_pi_lightcone_jk_multiprocessing(self, dataset_name, jackknife_region_indices_pos,
		...
		file_temp = h5py.File(f"{temp_file_path}/w_temp_data_{figname_dataset_name}.hdf5", "w")
		keys = []
	...
	def _count_pairs_xi_rp_pi_lightcone_jk_multiprocessing(self, dataset_name, jackknife_region_indices_pos,
														   jackknife_region_indices_shape, temp_file_path,
	...
	def _legacy_measure_xi_rp_pi_lightcone_jk_brute(self, dataset_name, jackknife_region_indices_pos,
											 jackknife_region_indices_shape, masks=None, return_output=False,
											 print_num=True, over_h=False, cosmology=None, jk_group_name="",
											 data_suffix="_SplusD", chunk_size=1000, num_nodes=1, temp_file_path=None
											 ):
	...
	def _legacy_measure_xi_rp_pi_lightcone_jk_tree(self, dataset_name, jackknife_region_indices_pos,
											jackknife_region_indices_shape, masks=None, return_output=False,
											print_num=True, over_h=False, cosmology=None, jk_group_name="",
											data_suffix="_SplusD", chunk_size=1000, num_nodes=1, temp_file_path=None
											):
				...
				if len(ind_rbin_i[n]) > 0:
					...
					shape_mask = \
						np.where(
							...
							0]
					np.add.at(Splus_D_jk, (jackknife_region_indices_pos_i[n], ind_r, ind_pi),
					...
					del e_plus, e_cross
					np.add.at(DD_jk, (jackknife_region_indices_pos_i[n], ind_r, ind_pi),
	...
	def _legacy_measure_xi_rp_pi_lightcone_jk_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures components of $\xi_{gg}$ and $\xi_{g+}$ in (rp,pi) bins including jackknife realisations for a batch
	...
	def _legacy_measure_xi_rp_pi_lightcone_jk_multiprocessing(self, dataset_name, jackknife_region_indices_pos,
													   jackknife_region_indices_shape, temp_file_path,
													   masks=None, return_output=False, jk_group_name="", over_h=False,
													   cosmology=None,
													   chunk_size=1000, num_nodes=1, data_suffix="_SplusD"
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_measure_xi_rp_pi_lightcone_jk_batch, indices)
	...
	def _legacy_count_pairs_xi_rp_pi_lightcone_jk_brute(self, dataset_name, jackknife_region_indices_pos,
												 jackknife_region_indices_shape, masks=None, return_output=False,
												 print_num=True, over_h=False, cosmology=None, data_suffix="_DD",
												 jk_group_name="", chunk_size=1000, num_nodes=1, temp_file_path=None
												 ):
	...
	def _legacy_count_pairs_xi_rp_pi_lightcone_jk_tree(self, dataset_name, jackknife_region_indices_pos,
												jackknife_region_indices_shape, masks=None, return_output=False,
												print_num=True, over_h=False, cosmology=None, jk_group_name="",
												data_suffix="_DD", chunk_size=1000, num_nodes=1, temp_file_path=None
												):
	...
	def _legacy_count_pairs_xi_rp_pi_lightcone_jk_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures components of $\xi_{gg}$ and $\xi_{g+}$ in (rp,pi) bins including jackknife realisations for a batch
		of indices from i to i+chunk_size. Support function for _measure_xi_rp_pi_box_jk_multiprocessing().
	...
	def _legacy_count_pairs_xi_rp_pi_lightcone_jk_multiprocessing(self, dataset_name, jackknife_region_indices_pos,
														   jackknife_region_indices_shape, temp_file_path,
														   masks=None, return_output=False, jk_group_name="",
														   over_h=False,
														   cosmology=None,
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_count_pairs_xi_rp_pi_lightcone_jk_batch, indices)
