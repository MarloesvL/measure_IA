...
class MeasureMultipolesBox(MeasureIABase, ReadData):
	...
	def _measure_xi_r_pi_box_brute(self, dataset_name, masks=None, rp_cut=None, return_output=False,
		...
		if (self.output_file_name != None) & return_output == False:
			...
			group = create_group_hdf5(output_file,
									  f"{self.snap_group}/multipoles/xi_g_plus/{jk_group_name}")
			...
			group = create_group_hdf5(output_file,
									  f"{self.snap_group}/multipoles/xi_g_cross/{jk_group_name}")
	...
	def _legacy_measure_xi_r_mur_box_brute(self, dataset_name, masks=None, rp_cut=None, return_output=False, jk_group_name="",
									ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions, $\xi_{gg}$ and $\xi_{g+}$, in (r, mu_r) bins for an object
		created with MeasureIABox. Uses 1 CPU.
	...
	def _legacy_measure_xi_r_mur_box_tree(self, dataset_name, masks=None, rp_cut=None, return_output=False, jk_group_name="",
								   ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions, $\xi_{gg}$ and $\xi_{g+}$, in (r, mu_r) bins for an object
		created with MeasureIABox. Uses 1 CPU. Uses KDTree for speedup.
	...
	def _legacy_measure_xi_r_mur_box_batch(self, i):
	...
	def _legacy_measure_xi_r_mur_box_multiprocessing(self, dataset_name, temp_file_path, masks=None,
											  rp_cut=None, return_output=False, jk_group_name="",
											  chunk_size=100, num_nodes=1, ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions, $\xi_{gg}$ and $\xi_{g+}$, in (r, mu_r) bins for an object
		created with MeasureIABox. Uses >1 CPU. Uses KDTree for speedup.
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_measure_xi_r_mur_box_batch, indices)
	...
	def _legacy_count_pairs_xi_r_mur_box_brute(self, dataset_name, masks=None, rp_cut=None, return_output=False,
										jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, in (r, mu_r) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_r_mur_box_brute for corr_type='gg': skips all shape/ellipticity computation.
		Uses 1 CPU.
	...
	def _legacy_count_pairs_xi_r_mur_box_tree(self, dataset_name, masks=None, rp_cut=None, return_output=False,
									   jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, in (r, mu_r) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_r_mur_box_tree for corr_type='gg': skips all shape/ellipticity computation.
		Uses 1 CPU. Uses KDTree for speedup.
	...
	def _legacy_count_pairs_xi_r_mur_box_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures the weighted pair counts DD in (r, mu_r) bins for a batch of indices from i to i+chunk_size.
	...
	def _legacy_count_pairs_xi_r_mur_box_multiprocessing(self, dataset_name, temp_file_path, masks=None, rp_cut=None,
												  return_output=False, jk_group_name="", num_nodes=1,
												  chunk_size=1000):
		r"""Pre-kernel reference copy for the A/B harness. Measures the clustering, $\xi_{gg}$, in (r, mu_r) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_r_mur_box_multiprocessing for corr_type='gg': skips all shape/ellipticity
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_count_pairs_xi_r_mur_box_batch, indices)
