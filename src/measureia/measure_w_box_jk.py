...
class MeasureWBoxJackknife(MeasureIABase, ReadData):
	...
	def _legacy_measure_xi_rp_pi_box_jk_brute(self, dataset_name, L_subboxes, masks=None, return_output=False,
									   jk_group_name="", ellipticity='distortion'):
	...
	def _legacy_measure_xi_rp_pi_box_jk_tree(self, dataset_name, L_subboxes, masks=None, return_output=False,
									  jk_group_name="", ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions including jackknife realisations, $\xi_{gg}$ and $\xi_{g+}$,
		in (rp, pi) bins for an object created with MeasureIABox. Uses 1 CPU. Uses KDTree for speedup.
	...
	def _legacy_measure_xi_rp_pi_box_jk_batch(self, i):
	...
	def _legacy_measure_xi_rp_pi_box_jk_multiprocessing(self, dataset_name, L_subboxes, temp_file_path,
												 masks=None, return_output=False, jk_group_name="",
												 chunk_size=1000, num_nodes=1, ellipticity='distortion'
												 ):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions including jackknife realisations, $\xi_{gg}$ and $\xi_{g+}$,
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_measure_xi_rp_pi_box_jk_batch, indices)
	...
	def _legacy_count_pairs_xi_rp_pi_box_jk_brute(self, dataset_name, L_subboxes, masks=None, return_output=False,
										   jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, including jackknife realisations in (rp, pi) bins for an
		object created with MeasureIABox. DD-only twin of _measure_xi_rp_pi_box_jk_brute for corr_type='gg':
		skips all shape/ellipticity computation. Uses 1 CPU.
	...
	def _legacy_count_pairs_xi_rp_pi_box_jk_tree(self, dataset_name, L_subboxes, masks=None, return_output=False,
										  jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, including jackknife realisations in (rp, pi) bins for an
		object created with MeasureIABox. DD-only twin of _measure_xi_rp_pi_box_jk_tree for corr_type='gg':
		skips all shape/ellipticity computation. Uses 1 CPU. Uses KDTree for speedup.
	...
	def _legacy_count_pairs_xi_rp_pi_box_jk_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures the weighted pair counts DD in (rp,pi) bins including jackknife realisations for a batch of
		indices from i to i+chunk_size. Support function for _count_pairs_xi_rp_pi_box_jk_multiprocessing().
	...
	def _legacy_count_pairs_xi_rp_pi_box_jk_multiprocessing(self, dataset_name, L_subboxes, temp_file_path,
													 masks=None, return_output=False, jk_group_name="",
													 chunk_size=1000, num_nodes=1
													 ):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, including jackknife realisations in (rp, pi) bins for an
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_count_pairs_xi_rp_pi_box_jk_batch, indices)
