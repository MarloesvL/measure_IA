...
class MeasureWLightcone(MeasureIABase):
	...
	def _measure_xi_rp_pi_lightcone_batch(self, i):
		...
		_measure_xi_rp_pi_lightcone_jk_batch; this one carries no patch indices and no
	...
	def _measure_xi_rp_pi_lightcone_multiprocessing(self, dataset_name, temp_file_path, masks=None,
		...
		The single-process twins are _measure_xi_rp_pi_lightcone_tree /
		_..._brute; this mirrors _measure_xi_rp_pi_lightcone_jk_multiprocessing
		...
		try:
			...
			with worker_pool.active_pool(num_nodes) as p:
				result = p.map(self._measure_xi_rp_pi_lightcone_batch, indices)
			...
			if os.path.exists(f"{temp_file_path}/w_temp_data_{figname_dataset_name}.hdf5"):
				...
				os.remove(f"{temp_file_path}/w_temp_data_{figname_dataset_name}.hdf5")
	...
	def _count_pairs_xi_rp_pi_lightcone_batch(self, i):
		r"""(sky) DD-only full-sample position-sample batch worker. As
		_measure_xi_rp_pi_lightcone_batch but with shapes=False, so no ellipticity
	...
	def _count_pairs_xi_rp_pi_lightcone_multiprocessing(self, dataset_name, temp_file_path, masks=None,
														 return_output=False, over_h=False, cosmology=None,
														 chunk_size=1000, num_nodes=1, data_suffix="_DD"):
		r"""Full-sample (no jackknife) sky pair count across ``num_nodes`` processes.

		DD-only twin of _measure_xi_rp_pi_lightcone_multiprocessing; see that method
		...
		try:
			...
			with worker_pool.active_pool(num_nodes) as p:
				result = p.map(self._count_pairs_xi_rp_pi_lightcone_batch, indices)
