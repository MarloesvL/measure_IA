...
class MeasureWBox(MeasureIABase, ReadData):
	...
	def _measure_xi_rp_pi_box_brute(self, dataset_name, masks=None,
		...
		created with MeasureIABox. Uses 1 CPU (brute-force full cross-join, no KDTree).

		The pair-counting loop is delegated to ``pair_kernel.accumulate`` (box geometry,
		``BoxRpPi`` binning, shape-chunked order, brute-force cross-join); the analytic-RR reduction and HDF5
	...
	def _legacy_measure_xi_rp_pi_box_brute(self, dataset_name, masks=None, return_output=False,
									jk_group_name="", ellipticity='distortion'):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected correlation functions, $\xi_{gg}$ and $\xi_{g+}$, in (rp, pi) bins for an object
	...
	def _count_pairs_xi_rp_pi_box_brute(self, dataset_name, masks=None, return_output=False, jk_group_name=""):
		r"""Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_brute for corr_type='gg': skips all shape/ellipticity computation.
		Uses 1 CPU (brute-force full cross-join). The counting loop is delegated to
		``pair_kernel.accumulate`` (shapes=False, backend='brute'); the RR reduction and writing are unchanged.
	...
	def _count_pairs_xi_rp_pi_box_tree(self, dataset_name, masks=None, return_output=False, jk_group_name=""):
		r"""Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_tree for corr_type='gg': skips all shape/ellipticity computation.
		Uses 1 CPU. Uses KDTree for speedup. The counting loop is delegated to
		``pair_kernel.accumulate`` (shapes=False, backend='tree'); the RR reduction and writing are unchanged.
	...
	def _count_pairs_xi_rp_pi_box_batch(self, i):
		r"""Measures the weighted pair counts DD in (rp,pi) bins for the shape-sample batch of indices
		from i to i+chunk_size. Support function for _count_pairs_xi_rp_pi_box_multiprocessing().

		Reads the sample arrays from shared memory and delegates the counting loop to
		``pair_kernel.accumulate`` (shapes=False, reusing the parent's shared self.pos_tree). Kept a
		bound method so it can be pickled into the spawned workers.
		...
		-------
		ndarray
			DD for this batch (summed over batches by the caller).
		"""
		if i + self.chunk_size > self.Num_shape_masked:
			i2 = self.Num_shape_masked
		else:
	...
	def _count_pairs_xi_rp_pi_box_multiprocessing(self, dataset_name, temp_file_path, masks=None,
												  return_output=False, jk_group_name="", num_nodes=1, chunk_size=1000):
		r"""Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_multiprocessing for corr_type='gg': skips all shape/ellipticity
		computation. Uses >1 CPU. Uses KDTree for speedup. The counting loop is delegated to
		``pair_kernel.accumulate`` via the batch worker; the mp orchestration, RR reduction and writing are unchanged.
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._count_pairs_xi_rp_pi_box_batch, indices)
	...
	def _legacy_count_pairs_xi_rp_pi_box_brute(self, dataset_name, masks=None, return_output=False, jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_brute for corr_type='gg': skips all shape/ellipticity computation.
		Uses 1 CPU.
	...
	def _legacy_count_pairs_xi_rp_pi_box_tree(self, dataset_name, masks=None, return_output=False, jk_group_name=""):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_tree for corr_type='gg': skips all shape/ellipticity computation.
	...
	def _legacy_count_pairs_xi_rp_pi_box_batch(self, i):
		r"""Pre-kernel reference copy for the A/B harness. Measures the weighted pair counts DD in (rp,pi) bins for a batch of indices from i to i+chunk_size.
		Support function for _count_pairs_xi_rp_pi_box_multiprocessing().
	...
	def _legacy_count_pairs_xi_rp_pi_box_multiprocessing(self, dataset_name, temp_file_path, masks=None,
												  return_output=False, jk_group_name="", num_nodes=1, chunk_size=1000):
		r"""Pre-kernel reference copy for the A/B harness. Measures the projected clustering, $\xi_{gg}$, in (rp, pi) bins for an object created with MeasureIABox.
		DD-only twin of _measure_xi_rp_pi_box_multiprocessing for corr_type='gg': skips all shape/ellipticity
		...
		try:
			...
			mp.set_start_method("spawn", force=True)
			with Pool(num_nodes) as p:
				result = p.map(self._legacy_count_pairs_xi_rp_pi_box_batch, indices)
